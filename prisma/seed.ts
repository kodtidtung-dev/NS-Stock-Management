...
async function main() {
  ...
  // สร้าง Products - ใช้ upsert เพื่อไม่ให้เกิด duplicate
  const productData = [
  ...
  // สร้าง Sample Stock Logs - ตรวจสอบการมีอยู่ก่อน
  ...
  // ตรวจสอบ stock logs ที่มีอยู่แล้ว
  const existingStockLogs = await prisma.stockLog.findMany({
    where: {
      date: {
        in: [yesterday, today]
      }
  ...
  // ถ้าไม่มี stock logs สำหรับวันที่เหล่านี้ ให้สร้างใหม่
  if (existingStockLogs.length === 0) {
